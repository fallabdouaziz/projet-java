...
public class AllClassImpl<T> implements IAllClass<T> {
	EntityManager em;
	public AllClassImpl() {
	...
	public int delete(int id) {
		try {
			em.getEntityManagerFactory();
			em.getTransaction().begin();
			  em.remove(em.createNamedQuery("T.findById").setParameter("id", id).getSingleResult());
			em.getTransaction().commit();
			return 1;	
	...
	public List<T> getAllUserToles(int id) {
		// TODO Auto-generated method stub
		return em.createNamedQuery("T.findById").setParameter("id", id).getResultList();
